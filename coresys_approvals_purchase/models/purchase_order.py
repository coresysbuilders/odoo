...
class PurchaseOrder(models.Model):
    ...
    # Single source of truth for the states that mean "already confirmed".
    # Both 'purchase' and the locked/consumed 'done' count — the gate, the two
    # backstops and _approval_is_consumed all read this set so a raw transition
    # into either state is treated identically (no lone 'purchase' literal).
    _approval_confirmed_states = frozenset({'purchase', 'done'})
    ...
    def create(self, vals_list):
        # Create-bypass backstop: catch create({'state': 'purchase'}) and
        # create({'state': 'done'}); the raise rolls the create back.
...
class PurchaseOrderLine(models.Model):
    ...
    _APPROVAL_LINE_WATCH = {
        'product_qty', 'price_unit', 'taxes_id', 'product_id', 'discount'}
