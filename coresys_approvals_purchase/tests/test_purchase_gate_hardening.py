...
class TestPurchaseGateHardening(TransactionCase):
    ...
    ambiguity, the create bypass for both 'purchase' and 'done', the PO-line read
    ...
    def test_line_edit_invalidates_approval(self):
        ...
        self.buyer.write({'groups_id': [(3, self.purchase_user_group.id)]})
        ...
        self.buyer.write({'groups_id': [(4, self.purchase_user_group.id)]})
        self.env.invalidate_all()
        self.env.registry.clear_cache()
    ...
    def test_create_in_done_state_blocked(self):
        # Round-2 HIGH: the create backstop covers every confirmed state — a
        # create({'state': 'done'}) is rejected too, not just 'purchase'.
        with self.env.cr.savepoint():
            with self.assertRaises(UserError):
                self.env['purchase.order'].with_user(self.buyer).create({
                    'partner_id': self.supplier.id,
