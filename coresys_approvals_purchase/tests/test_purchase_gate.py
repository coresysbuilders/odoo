...
class TestPurchaseGate(TransactionCase):
    ...
    def test_done_state_write_blocked(self):
        # Round-2 HIGH: the LOCKED/consumed 'done' state is gated exactly like
        # 'purchase' — the confirmed-state gate has no done-shaped hole.
        po = self._gated_po()
        with self.env.cr.savepoint():
            with self.assertRaises(UserError):
                po.with_user(self.buyer).write({'state': 'done'})
        po.invalidate_recordset(['state'])
